...
class Board:
	...
	def move(self, action: tuple):
		while(self.check_bounder(action)):
			if action[1] == 'l': self.robots[action[0]][1] -= 1
			if action[1] == 'r': self.robots[action[0]][1] += 1
			if action[1] == 'u': self.robots[action[0]][0] -= 1
			if action[1] == 'd': self.robots[action[0]][0] += 1
